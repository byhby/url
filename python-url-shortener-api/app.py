...
<head>
  <meta charset="UTF-8">
  <title>Python URL Shortener API</title>
  <style>
...
<body>
  <div class="card">
    <h1>🚀 Python URL Shortener API</h1>
...
<body>
  <div class="box">
    ...
    # 4. REDIRECTION: /{5_digit_code}
    code_match = re.match(r"^/([a-zA-Z0-9]{5})/?$", request.url.path)
    if code_match:
...
<body style="background:#0f172a; color:#fff; text-align:center; padding:50px; font-family:sans-serif;">
  ...
  <p style="color:#94a3b8;">The requested short link does not exist.</p>
        ...
        if not bypass and not shortener.is_allowed_in_app_browser(user_agent):
            return HTMLResponse(shortener.get_restricted_browser_html(), status_code=403)
