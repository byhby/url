...
def render_url_created_html(short_url: str, record: dict) -> str:
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .card {{ background: #1e293b; border-radius: 20px; padding: 36px 28px; max-width: 480px; width: 100%; text-align: center; border: 1px solid #334155; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); }}
    .icon {{ font-size: 48px; margin-bottom: 16px; }}
    h2 {{ font-size: 22px; font-weight: 700; color: #38bdf8; margin-bottom: 8px; }}
    p.subtitle {{ font-size: 14px; color: #94a3b8; margin-bottom: 24px; }}
    .url-box {{ background: #0f172a; border: 2px dashed #0284c7; border-radius: 12px; padding: 14px 18px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; transition: all 0.2s ease; margin-bottom: 16px; }}
    .url-box:hover {{ border-color: #38bdf8; background: #111e38; }}
    .url-text {{ font-size: 16px; font-weight: 600; color: #f8fafc; word-break: break-all; text-align: left; }}
    .copy-badge {{ background: #0284c7; color: #fff; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; flex-shrink: 0; margin-left: 10px; }}
    .toast {{ display: none; background: #22c55e; color: #fff; font-size: 13px; font-weight: 600; padding: 8px 12px; border-radius: 8px; margin-bottom: 16px; }}
    .actions {{ display: flex; flex-direction: column; gap: 12px; margin-top: 16px; }}
    .btn-visit {{ background: linear-gradient(135deg, #0284c7, #2563eb); color: #ffffff; text-decoration: none; padding: 14px 20px; border-radius: 12px; font-weight: 700; font-size: 16px; display: block; transition: transform 0.1s ease, box-shadow 0.2s ease; }}
    .btn-visit:hover {{ transform: translateY(-2px); box-shadow: 0 10px 20px -5px rgba(2, 132, 199, 0.4); }}
    .info-meta {{ font-size: 13px; color: #64748b; margin-top: 24px; border-top: 1px solid #334155; padding-top: 16px; text-align: left; line-height: 1.6; }}
  </style>
</head>
<body>
  <div class="card">
    <div class="icon">✨</div>
    <h2>Short URL Ready!</h2>
    <p class="subtitle">Click the short link below to copy it automatically</p>

    <div id="toast" class="toast">✓ Copied to clipboard!</div>

    <div class="url-box" onclick="copyShortUrl()">
      <span class="url-text" id="shortUrlText">{short_url}</span>
      <span class="copy-badge" id="copyBtn">Click to Copy</span>
    </div>

    <div class="actions">
      <a href="{short_url}" target="_blank" class="btn-visit">🌐 Visit Site</a>
    </div>

    <div class="info-meta">
      <div><strong>Destination:</strong> {record['destinationUrl']}</div>
      <div><strong>Validity:</strong> {record['validityDays']} Days (Expires: {record['expiresAt']})</div>
    </div>
  </div>
  ...
  <script>
    function copyShortUrl() {{
      ...
      navigator.clipboard.writeText(text).then(() => {{
        ...
        const badge = document.getElementById('copyBtn');
...
async def handle_custom_routes(request: Request, path_name: str):
    ...
    if create_match:
        try:
            ...
            return HTMLResponse(render_url_created_html(short_url, record), status_code=201)
